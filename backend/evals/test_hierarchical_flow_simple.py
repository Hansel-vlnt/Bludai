...
def main():
    print("Starting Headless Hierarchical Workflow Simulation (Simple Path)...")
    
    prompt = (
        "[CRITICAL INSTRUCTION FOR SUPERVISOR: You MUST NOT write the code yourself. "
        "You MUST output a JSON response with next_node set to 'Developer'.]\n\n"
        "[CRITICAL INSTRUCTION FOR DEVELOPER: Do NOT use tools. Just return 'HELLO_WORLD' and say task complete.]"
    )
    
    inputs = {"messages": [HumanMessage(content=prompt)]}
    config = {"configurable": {"thread_id": "sim_test_simple_001"}}
